// Copied from https://raw.githubusercontent.com/blakeembrey/pluralize and converted to TS

type Rule = [RegExp, string];
type IrregularRules = { [key: string]: string };
type Uncountables = { [key: string]: boolean };

// Rule storage - pluralize and singularize need to be run sequentially,
// while other rules can be optimized using an object for instant lookups.
var pluralRules: Rule[] = [];
var singularRules: Rule[] = [];
var uncountables: Uncountables = {};
var irregularPlurals: IrregularRules = {};
var irregularSingles: IrregularRules = {};
 ...
 */
function interpolate(str: string, args: [string, number]) {
...
function replace(word: string, rule: Rule): string {
  return word.replace(rule[0], function(match, index) {
    var result = interpolate(rule[1], [match, index]);
 ...
 */
function sanitizeWord(token: string, word: string, rules: Rule[]) {
 ...
 */
function replaceWord(replaceMap: IrregularRules, keepMap: IrregularRules, rules: Rule[]) {
 ...
 */
function checkWord(replaceMap: IrregularRules, keepMap: IrregularRules, rules: Rule[]) {
  return function(word: string) {
    var token = word.toLowerCase();
